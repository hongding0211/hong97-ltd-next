# walkcalc-ledger-backend Specification
...
SHALL keep exact integer-cent arithmetic as an internal backend detail.
...
The system SHALL store WalkCalc groups, participants, records, and participant
projections separately so queries can use backend indexes instead of loading
full group documents.
...
The system SHALL maintain a participant projection for each participant in each
group and SHALL use it as the authoritative source for balances and participant
statistics.
...
surfaces without requiring clients to infer full-data facts from paginated
records.
