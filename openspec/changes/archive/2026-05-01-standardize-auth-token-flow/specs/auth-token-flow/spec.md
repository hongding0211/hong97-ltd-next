...
The system SHALL issue a short-lived access token and a longer-lived refresh credential when a user logs in successfully.
...
The system SHALL authenticate protected API requests with a valid access token only.
